        ...
        <div class="col-md-6 col-lg-4 animated zoomIn">
            <div class="d-box position-relative">
                ...
                <div class="flex-mode mt-5 justify-content-between">
                    ...
                    <p class="flex-text">{{ !empty($document->customer) ? $document->customer->name : \App\Helpers\GeneralHelper::EMPTY_DASHES() }}</p>
                </div>
                ...
                <div class="flex-mode justify-content-between">
                    ...
                    <p class="flex-text">{{ !empty($document->customer->email) ? $document->customer->email : \App\Helpers\GeneralHelper::EMPTY_DASHES() }}</p>
                </div>
                <div class="flex-mode justify-content-between">
                    <p class="flex-heading">{{ __('Phone') }}</p>
                    <p class="flex-text">{{ !empty($document->customer->mobile_number) ? $document->customer->mobile_number : \App\Helpers\GeneralHelper::EMPTY_DASHES() }}</p>
                </div>
                <div class="flex-mode justify-content-between">
                    <p class="flex-heading">{{ __('Status') }}</p>
                    <p class="flex-text">
                        <span class="badge {{ \App\Helpers\GeneralHelper::USER_DOCUMENT_STATUS_CLASS($document->status) }}">
