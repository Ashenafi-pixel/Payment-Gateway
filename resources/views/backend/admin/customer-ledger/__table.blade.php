<div class="table-responsive table-nowrap d-box p-0 border-radius-0">
    <table class="table">
        ...
        @forelse($allTransactions as $ledger)
            <tr>
                <td>{{ $i++ }}</td>
                <td>{{ !empty($ledger->customer) ? $ledger->customer->name : \App\Helpers\GeneralHelper::EMPTY_DASHES() }}</td>
                <td>{{ !empty($ledger->transactionType) ? \App\Helpers\GeneralHelper::STATUS_CASING($ledger->transactionType->name) : \App\Helpers\GeneralHelper::EMPTY_DASHES() }}</td>
