...
@section('content')
    ...
    {!! Form::open(['url' => route(\App\Helpers\IUserRole::ADMIN_ROLE.'.approve.document',['type' => 'merchant']),'class' => 'ajax']) !!}
    <div class="row gy-4">
        <div class="col-lg-4 card-border">
            <div class="card document">
                ...
                @else
                    <div class="card-body mh-185 p-0 text-center">
        ...
        @if($user->is_school == \App\Helpers\IUserStatuses::IS_SCHOOL)
            <div class="col-lg-4 card-border">
                <div class="card document">
                    <div class="card-header text-center">
                        <h3 class="sub-title mb-0">{{ __('Registration Form') }}</h3>
                    </div>
                    @if(pathinfo($documents->document_detail->registration_form_doc, PATHINFO_EXTENSION) == \App\Helpers\IDocTypes::PDF)
                        <div class="card-body p-0 mh-185">
                            <iframe width="100%" height="185" src="{{asset($documents->document_detail->registration_form_doc)}}"
                                    frameborder="0"></iframe>
                        </div>
                    @else
                        <div class="card-body p-0 mh-185 text-center">
                            <img class="img-fluid" src="{{asset($documents->document_detail->registration_form_doc)}}" alt="">
                        </div>
                    @endif
                    <div class="card-footer text-end p-2">
                        <div class="row">
                            <div class="col-12 text-center">
                                <input type="radio" class="btn-check" value="{{ \App\Helpers\IStatuses::APPROVED }}" name="registration_form_doc" id="registration_approve-pdf" autocomplete="off" @if($documents->document_detail->registration_form_doc_status == \App\Helpers\IStatuses::APPROVED) checked @endif>
                                <label class="btn btn-outline-success box-shadow-0" for="registration_approve-pdf">Approved</label>

                                <input type="radio" class="btn-check" value="{{ \App\Helpers\IStatuses::REJECTED }}" name="registration_form_doc" id="registration_reject-pdf" autocomplete="off" @if($documents->document_detail->registration_form_doc_status == \App\Helpers\IStatuses::REJECTED) checked @endif>
                                <label class="btn btn-outline-danger box-shadow-0" for="registration_reject-pdf">Rejected</label>
                            </div>
                        </div>
                    </div>
                </div>
